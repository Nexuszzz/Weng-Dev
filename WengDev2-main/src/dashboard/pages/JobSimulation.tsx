      ...
      <div className="max-w-7xl mx-auto">
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                  selectedCategory === cat.name.toLowerCase() ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
              >
                <cat.icon size={18} />
            ...
            {['All', 'Beginner', 'Intermediate', 'Advanced'].map(level => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                  selectedLevel === level.toLowerCase() ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
              >
                {labelLevel(level)}
              </button>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSimulations.map((sim) => (
            <div key={sim.id} className="bg-white rounded-xl p-6 hover:shadow-lg transition-shadow">
