...
const OAuthMock: React.FC = () => {
  ...
  const brand = useMemo(() => {
    ...
    return {
      name: 'Google',
      color: '#4285F4',
      bg: 'from-gray-900 via-gray-800 to-emerald-900',
      logo: <GoogleLogo />,
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${brand.bg} text-white flex items-center justify-center p-6 relative`}>
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="w-full max-w-md">
        ...
        <div className="bg-gray-900/70 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/10 p-8">
          ...
          <div className="flex items-center gap-2 text-xs text-emerald-300/90 mb-4">
          ...
          <form onSubmit={submit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-3 rounded-lg bg-gray-800/80 border border-white/10 focus:border-emerald-500 outline-none"
              />
            </div>
            {provider === 'linkedin' ? (
              <div>
