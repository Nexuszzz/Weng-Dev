...
const Settings: React.FC = () => {
  ...
  const Toggle: React.FC<{label:string; value:boolean; onChange:(v:boolean)=>void; description?:string; icon?:React.ReactNode}> = ({label, value, onChange, description, icon}) => (
    <div className="flex items-start justify-between py-4 border-b last:border-b-0">
      ...
      <button
        ...
        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 ${value? 'bg-emerald-600':'bg-gray-300'}`}
      >
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 space-y-8">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 divide-y">
        <h2 className="text-sm font-semibold text-gray-900 mb-2">Tampilan & Tema</h2>
        <Toggle
          label="Mode Gelap"
          value={settings.darkMode}
          onChange={v=>updateSettings({darkMode:v})}
          description="Aktifkan tema gelap secara global (demo visual saja)."
          icon={<Moon size={16} className="text-gray-500" />}
        />
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 divide-y">
        ...
        <div className="py-4 border-b last:border-b-0 flex items-center justify-between">
          ...
          <select
            ...
            className="px-3 py-2 rounded-lg border bg-white text-sm focus:ring-emerald-500 focus:outline-none"
          >
