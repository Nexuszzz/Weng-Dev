...
import { Mail, Lock, Loader2, ArrowLeft, Linkedin, AtSign, Eye, EyeOff, AlertCircle, Building2 } from 'lucide-react';
...
const Login: React.FC = () => {
  ...
  return (
    <div
      ...
      className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-emerald-900 text-white flex items-center justify-center p-6 relative"
    >
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div style={{ x: sx1, y: sy1 }} className="absolute -top-10 -left-10 w-72 h-72 bg-emerald-500/10 rounded-full blur-3xl animate-pulse" />
        <motion.div style={{ x: sx2, y: sy2 }} className="absolute -bottom-10 -right-10 w-96 h-96 bg-blue-500/10 rounded-full blur-3xl animate-bounce" />
      </div>
      ...
      <motion.div className="w-full max-w-md relative" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
        ...
        <div className="bg-gray-900/70 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/10 p-8 hover:shadow-emerald-500/10 transition-shadow">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400 mt-1">Mulai lagi perjalanan karirmu</p>
          </div>
          ...
          <div className="my-4 flex items-center gap-4">
            <div className="h-px flex-1 bg-white/10" />
            <span className="text-xs text-gray-400">demo akses</span>
            <div className="h-px flex-1 bg-white/10" />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <motion.button
              onClick={() => { mockLogin(); navigate('/dashboard'); }}
              disabled={loading}
              whileTap={{ scale: 0.98 }}
              whileHover={{ y: -1 }}
              className="flex items-center justify-center gap-2 py-2.5 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 transition-all"
            >
              <AtSign className="w-4 h-4" />
              <span className="text-sm">Demo Kandidat</span>
            </motion.button>
            <motion.button
              onClick={() => { mockCompanyLogin(); navigate('/company'); }}
              disabled={loading}
              whileTap={{ scale: 0.98 }}
              whileHover={{ y: -1 }}
              className="flex items-center justify-center gap-2 py-2.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-all"
            >
              <Building2 className="w-4 h-4" />
              <span className="text-sm">Demo Perusahaan</span>
            </motion.button>
          </div>

          <p className="text-center text-sm text-gray-400 mt-6">
            Belum punya akun?{' '}
            <Link to="/register" className="text-emerald-400 hover:text-emerald-300 font-semibold">Daftar</Link>
          </p>
        </div>
      </motion.div>
    </div>
